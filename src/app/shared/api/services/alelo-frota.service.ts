...
class AleloFrotaService extends __BaseService {
  ...
  carsUsingGETResponse(
    params?: AleloFrotaService.FilterParams
  ): __Observable<__StrictHttpResponse<Car[]>> {
    let url =
      this.rootUrl +
      `vehicle?${params?.filter ? 'plate=' + params.filter : ''}${
  ...
  carUsingGET(params: AleloFrotaService.FindCarParams): __Observable<Car> {
    return this.carUsingGETResponse(params).pipe(__map((_r) => _r.body as Car));
  }

  carUsingGETResponse(
    params: AleloFrotaService.FindCarParams
  ): __Observable<__StrictHttpResponse<Car>> {
    let req = new HttpRequest<any>(
      'GET',
      this.rootUrl + `/vehicle/${params.id}`
    );
  ...
  carUsingPOSTOrPUT(
    params: AleloFrotaService.SaveCarParams
  ): __Observable<Car> {
